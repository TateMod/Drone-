...
// Constructor now accepts a Pico I2C port and optional address/timeout:
VL53L1X(i2c_inst_t * i2c_port = i2c0,
        uint8_t i2c_address = 0x29,
        uint16_t io_timeout_ms = 1000);

void setI2CPort(i2c_inst_t * port) { _i2c_port = port; }
i2c_inst_t * getI2CPort() { return _i2c_port; }
...
uint16_t readRangeSingleMillimeters(bool blocking = true) { return readSingle(blocking); }
...
bool timeoutOccurred();
  ...
  private:
    ...
    // storage for values read from specific result registers
    struct ResultBuffer
    {
    ...
    // Replacing TwoWire *bus with a Pico i2c_inst_t * pointer
    ...
    // We'll replace timeout_start_ms with an absolute_time_t to use Pico's timing
    ...
    // Check if timeout is enabled (nonzero) and has expired
    bool checkTimeoutExpired()
    {
        if (io_timeout == 0) return false;  // no timeout set
        // how long has passed since startTimeout()
        uint64_t elapsed_ms = to_ms_since_boot(get_absolute_time()) 
                              - to_ms_since_boot(timeout_start);
        return (elapsed_ms > io_timeout);
    }
